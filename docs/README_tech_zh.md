...
- **MPPI-4D**：
  - 控制空间为 4 个车轮级变量（前左/后右转角 + 前左/后右轮速）；
  - 通过运动学转换获得 $(v_x, v_y, \omega)$ 与 8 维车轮指令；
  - 优点：对舵轮约束建模更精确，安全性/可行性最好；
  - 缺点：维度更高，采样效率下降，计算更重。

- **MPPI-H**（Hybrid）：
  - 同时维护 3D(b) 与 4D 两个控制器；
  - 根据当前跟踪误差（位置误差、航向误差）决定使用哪一种：
    - 误差小 → 3D(b) 模式（效率优先）；
    - 误差大 → 4D 模式（安全优先）；
  - 模式切换时将最优控制序列从一个子控制器传递给另一个，实现平滑过渡。
...
- `teb_local_planner.yaml`
  - 当前 `navigation.launch` 中标记为不推荐/未支持，但参数结构可作为参考，尤其是 footprint、障碍物距离、加速度/速度约束等设置方式。

### 5.4 MPPI 控制器内部参数

- 位置：对应控制包的源码中（如 `src/control/mppi_3d`、`mppi_4d`、`mppi_h` 下的 `param` / `*_setting` 文件）。
- 关键字段包括：
...
- 确认你的机器人满足：
  - 坐标系命名与朝向：`base_link` 前方为 +x，左为 +y，右手系；
  - TF 树存在 `map → odom → base_link`，并连续发布；
  - 舵轮几何参数（`l_f`、`l_r`、`d_l`、`d_r`、`tire_radius`）根据真实底盘修改，否则 MPPI 的运动学预测会与实际不符。
...
- 输出适配：
  - 若你的底盘不接受 `/cmd_vel` 而是车轮级命令，可参考 `vel_driver` 将 MPPI 的输出转换为适配你控制器的格式。
...
- 修改采样空间：
  - 若要支持新的控制维度或其他轮式布局，可仿照 `mppi_4d` 实现新的控制空间结构 + 正/逆运动学转换；
  - 注意保持统一的接口（最终仍输出 `/cmd_vel` 或车轮级命令）。

### 6.5 增加新的 MPPI 模式

- 以现有模式为模板：
  - 在 mppi_3d 或 mppi_4d 源码中增加新参数集合（如 `mppi_3d_c`），并在对应 launch 中添加配置入口；
  - 若要实现更复杂的切换逻辑，可在 `mppi_h` 的 Hybrid Core 中增加新的 mode 状态与切换条件。

- 经验建议：
  - 先在仿真中通过记录 `mppi_eval` 消息与可视化轨迹，观察代价各项与控制输出是否符合预期，再考虑上车实测。
...
- **仅参考算法实现：**
  - 建议重点阅读 `src/control` 与 `src/planning/reference_costmap_generator` 相关源码，将其中 MPPI 结构、车辆模型、代价函数移植到你自己的 ROS2/Nav2 或其它框架；
  - 配置部分可按当前 `config/` 目录为模板改写。

- **原样在 ROS1 中跑通：**
  - 建议不与 ROS2 工程混编，而是在单独的 ROS1 工作区或 Docker 容器中，按根目录 `Makefile` 的说明 `make install_deps && make build && make navigation` 即可。
